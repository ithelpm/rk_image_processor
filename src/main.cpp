...
static int v4l2_to_rga_fmt(uint32_t fourcc)
{
    ...
    case V4L2_PIX_FMT_NV12:
        return rga::fmt::YCB_CR_420_SP;
    case V4L2_PIX_FMT_NV16:
        return rga::fmt::YCB_CR_422_SP;
    case V4L2_PIX_FMT_BGR24:
        return rga::fmt::BGR_888;
    default:
        return -1;
    }
}
...
static DequeueResult camera_dequeue(Camera &cam)
{
    ...
    if (r <= 0)
    {
        std::fprintf(stderr, "[v4l2] poll timeout/error\n");
        return {};
    }
...
struct FrameTiming
{
    double rga_ms = 0, encode_ms = 0, write_ms = 0;
};
struct FrameStats
{
    double dequeue_ms, process_ms, total_ms,
...
static void print_stats(const std::vector<FrameStats> &stats)
{
    ...
    std::printf("\n=== 延遲統計（%zu 幀）===\n", stats.size());
    pr("dequeue", gather([](auto &s)
                         { return s.dequeue_ms; }));
    pr("rga", gather([](auto &s)
                     { return s.rga_ms; }));
    pr("encode", gather([](auto &s)
                        { return s.encode_ms; }));
    pr("write", gather([](auto &s)
                       { return s.write_ms; }));
    pr("process", gather([](auto &s)
                         { return s.process_ms; }));
    pr("total", gather([](auto &s)
                       { return s.total_ms; }));
}
